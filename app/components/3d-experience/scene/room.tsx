...
export const Room = memo(function RoomComponent(props: RoomProps) {
  ...
  // Configure texture immediately (before material creation)
  useMemo(() => {
    texture.flipY = false;
  ...
  const textureStandardMaterial = useMemo(
    () =>
  ...
  // Motion values stored in refs - created once and reused
  const motionValuesRef = useRef({ standard: useMotionValue(0), glass: useMotionValue(0) });

  // Configure all textures using a dedicated effect
  useEffect(() => {
    // Store ref for later use in video control
    textureDualMonitorLRef.current = textureDualMonitorL;

    // Dual monitor (L) texture configuration
    // Note: repeat is a Vector2, we need to set it as an object for React Compiler
    const repeatL = { x: -1, y: textureDualMonitorL.repeat.y };
    configureTexture(textureDualMonitorL, {
    ...
    // Dual monitor (R) texture configuration
    const repeatR = { x: -1, y: textureDualMonitorR.repeat.y };
    configureTexture(textureDualMonitorR, {
      wrapS: THREE.RepeatWrapping,
      repeat: repeatR,
      rotation: Math.PI / 2,
      center: new THREE.Vector2(0.5, 0.5),
    ...
    // MacBook Pro texture configuration
    const repeatMacBook = { x: 1, y: textureMacBookPro.repeat.y };
    configureTexture(textureMacBookPro, {
      flipY: false,
      wrapS: THREE.RepeatWrapping,
      repeat: repeatMacBook,
      rotation: Math.PI / 2,
      center: new THREE.Vector2(0.5, 0.5),
    });
  }, [textureDualMonitorL, textureDualMonitorR, textureMacBookPro]);

  useEffect(() => {
    // Animate Texture - values from stable refs
    animate(motionValuesRef.current.standard, section === 0 ? 1 : 0);
    animate(motionValuesRef.current.glass, section === 0 ? 1 : 0);
  }, [section]);
  ...
  const materialRefsRef = useRef({ standard: textureStandardMaterial, glass: textureGlassMaterial });

  useFrame(() => {
    // Update refs on each frame to ensure they're always current
    materialRefsRef.current.standard = textureStandardMaterial;
    materialRefsRef.current.glass = textureGlassMaterial;

    materialRefsRef.current.standard.opacity = motionValuesRef.current.standard.get();
    materialRefsRef.current.glass.opacity = motionValuesRef.current.glass.get();
  });
