...
!fs.existsSync(FONTS_DEFINITIONS) && fs.mkdirSync(FONTS_DEFINITIONS, { recursive: true });
    ...
    return new Promise(resolve => {
        ...
        https.get(FONT_KIT_URL, (response) => {
            ...
            response.on('end', () => {
                ...
                fileExists(`${FONTS_DEFINITIONS}/${fontFamily}.css`).then(res => {
                    if (res) {
            ...
            https.get(FONT_KIT_URL, { headers }, (response) => {
                ...
                response.on('end', () => {
                    for (const fontFaceRule of content.match(/@font-face {[^}]+}/g)) {
                        ...
                        const fontFamilyFolder = `${FONTS_FOLDER}/${fontFamily}`;

                        // Create the font family folder if it doesn't exist
                        if (!fs.existsSync(fontFamilyFolder)) {
                        ...
                        for (const fontUrlAndFormat of fontUrlAndFormats) {
                            ...
                            const filename = `${fontWeight}_${fontStyle}.${extension}`;
                            ...
                            if (!downloadedFonts[filename]) {
                                https.get(fontUrl, (response) => {
                                    ...
                                    response.pipe(fs.createWriteStream(`${FONTS_FOLDER}/${fontFamily}/${filename}`));
                                });
                            }
                        }
                    }
...
async function add_style_class(file_url, font_family){
    // Capitalize the font family name
    const font_family_name = font_family.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
    
    return new Promise(resolve => {
        let str = `.tk-${font_family} { font-family: "${font_family_name}", sans-serif; }`;
    ...
    download_fontfiles().then(() => {
        ...
        http_call().then((body) => {
            ...
            for (const fontFaceRule of body.match(/@font-face {[^}]+}/g)) {
                ...
                // Capitalize the font family name
                let font_family_name = fontFamily.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
                ...
                fs.appendFileSync(`${FONTS_DEFINITIONS}/${fontFamily}.css`, cssString, function (err) {
                    if (err) throw (err);
                });
            }

            // Now we should add the class definition to our css file
            add_style_class(`${FONTS_DEFINITIONS}/${FONT_FAMILY_NAME}.css`, FONT_FAMILY_NAME ).then((res) => {
                log(res);
            });
        });
    });
